...
sub main {
    ...
    #スコアのもととなるデータを算出
    &calc_rating( $dbh, $courses, $s_date, $e_date );
...
sub calc_rating {
    my ( $dbh, $courses, $s_date, $e_date ) = @_;
...
sub get_courses {
    my ($dbh) = @_;
    #my $sth = $dbh->prepare("SELECT course_id, course_status FROM courses");

    my $sql = "SELECT courses.course_id, courses.course_status, profs.prof_status";
    $sql .= " FROM courses";
    $sql .= " LEFT JOIN profs ON profs.prof_id=courses.prof_id";

    my $sth = $dbh->prepare($sql);
    $sth->execute();
    my $h = {};

    while ( my ( $id, $course_status, $prof_status ) = $sth->fetchrow_array ) {
        my $status = 0;
        if($course_status == 1 && $prof_status == 1) {
            $status = 1;
        }
