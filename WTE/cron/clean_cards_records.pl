...
sub main {
	...
	#cardsテーブルをクリーンナップ
	my $epoch = time - 86400;
	my $last_sql = "DELETE FROM cards WHERE crd_cdate<${epoch} AND crd_success=0 AND crd_rdate=0";
	my $deleted = 0;
	eval {
	...
	&loging("notice", "completed. ${deleted} records in cards table has been deleted.");
	exit;
}
