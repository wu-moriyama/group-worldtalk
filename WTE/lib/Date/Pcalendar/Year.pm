    ...
    if ($sign > 0)
    {
        ...
        while ($loop)
        {
                ...
                ($min == $index))
            {
                if ($max >= $limit)
                {
                    return( Date::Pcalc->new(++$year,1,1), $rest, +1 );
                }
                else { $index = $max + 1; }
            }
    ...
    else # ($sign < 0)
    {
        ...
        while ($loop)
        {
                ...
                ($max == $index))
            {
                if ($min <= 0)
                {
                    return( Date::Pcalc->new(--$year,12,31), $rest, -1 );
                }
                else { $index = $min - 1; }
            }
            if ($half->bit_test($index))
...
sub add_delta_workdays
{
    ...
    if ($sign == 0)
    {
        return( Date::Pcalc->new($yy,$mm,$dd), $days, 0 );
    }
    $days = -$days if ($days < 0);
    ...
    if ($sign > 0)
    {
        ...
        {
            $days -= $diff;
            return( Date::Pcalc->new(++$yy,1,1), $days, +1 );
        }
        # else ($days < $diff)
    ...
    else # ($sign < 0)
    {
        ...
        {
            $days -= $diff;
            return( Date::Pcalc->new(--$yy,12,31), -$days, -1 );
        }
        # else ($days < $diff)
        # Now calculate proportional jump (approximatively):
