package FCC::Action::Admin::CrdlstfrmAction;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	$params->{sort} = [ ['crd_id', 'DESC'] ];
	#カード決済情報を検索
	my $ocrd = new FCC::Class::Card(conf=>$self->{conf}, db=>$self->{db});
	my $res = $ocrd->get_list($params);
	#ページナビゲーション用リンクの共通パラメータ
	my @url_params = ("m=crdlstfrm");
	...
	#プラン情報を取得
	my $opln = new FCC::Class::Plan(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $pln_list = $opln->get_all();
	my $plns = {};
	for my $pln (@{$pln_list}) {
		$plns->{$pln->{pln_id}} = $pln;
	}
