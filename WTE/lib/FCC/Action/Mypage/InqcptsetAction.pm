package FCC::Action::Mypage::InqcptsetAction;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	$self->send_mail($in, $proc->{captions});
	#
	$context->{proc} = $proc;
	return $context;
}
...
sub send_mail {
	...
	#
	for my $tmpl_id ("mpg9011", "mpg9012") {
		my $t = $ot->get_template_object($tmpl_id);
		#置換
		while( my($k, $v) = each %{$in} ) {
			$t->param($k => $v);
			if($k =~ /^(inq_title)$/) {
				$t->param("${k}_caption" => $captions->{"${k}_caption_${v}"});
			}
		}
