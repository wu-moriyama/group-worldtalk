package FCC::Action::Mypage::LinlstfrmAction;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	#認証情報を検索
	my $olin = new FCC::Class::Login(conf=>$self->{conf}, db=>$self->{db});
	my $res = $olin->get_list($params);
	#ページナビゲーション用リンクの共通パラメータ
	my @url_params = ("m=linlstfrm");
