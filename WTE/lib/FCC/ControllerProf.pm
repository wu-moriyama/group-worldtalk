...
sub dispatch {
	...
	#認証処理
	my $session = new FCC::Class::ProfSession(conf=>$c, memd=>$memd, q=>$self->{q});
	...
	if($m !~ /^(Athlinfrm|Athlinsmt)$/i) {
		unless($session_data) {
			...
			print "Location: $c->{CGI_DIR_URL}/prof.cgi?m=athlinfrm\n\n";
			exit;
		}
	}
	#アクション（モデル）
