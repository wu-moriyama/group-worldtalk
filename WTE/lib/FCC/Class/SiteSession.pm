...
sub new {
	...
	#memcacheの有効期限（秒）
	$self->{expire} =3600;
	#マイページmemcacheのキープレフィックス
	$self->{mypage_prefix} = "mypage_";
	#マイページcookie名
	$self->{mypage_cookie_name} = "msid";
	#
	bless $self, $class;
	return $self;
}
...
sub logoff {
	...
	if($self->{data}) {
		...
		unless($mem) {
			my $msg = "failed to delete a session data from memcache. : sid=${sid}";
...
sub create {
	...
	unless($mem) {
		my $msg = "failed to set a session data to memcache. : sid=${sid}";
		FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
		croak $msg;
	}
	#
...
sub auth {
	...
	} elsif($cookies{$self->{mypage_cookie_name}}) {
		$sid = $cookies{$self->{mypage_cookie_name}}->value;
	} elsif($cookies{$self->{cookie_name}}) {
	...
	if($sid =~ /^(\d+)_([a-fA-F0-9]{32})$/) {
		my $member_id = $1;
		my $digest = $2;
		my $mem_key = $self->{mypage_prefix} . $member_id;
		my $mydata = $self->{memd}->get($mem_key);
		if( $mydata && ref($mydata) eq "HASH" && $mydata->{digest} && $mydata->{digest} eq $digest && $mydata->{member} ) {
			$data = $mydata->{member};
		}
		$self->{data} = $data;
		return $data;
	} elsif($sid =~ /^[a-fA-F0-9]{32}$/) {
		my $mem_key = $self->{prefix} . $sid;
		my $data = $self->{memd}->get($mem_key);
