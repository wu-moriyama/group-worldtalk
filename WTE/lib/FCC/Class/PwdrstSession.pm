package FCC::Class::PwdrstSession;
################################################################################
...
sub new {
	...
	$self->{prefix} = "pwdrst_";
	#cookie名
	$self->{cookie_name} = "prsid";
	#セッション有効秒数（memcache有効秒数）
	$self->{expire} = $self->{conf}->{pwdrst_session_expire};
	#
	bless $self, $class;
	return $self;
}
...
sub create {
	...
	unless($mem) {
		my $msg = "failed to set a reminder session data to memcache. : sid=${sid}";
		FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
		croak $msg;
	}
	#
...
sub update {
	...
	my $mem = $self->{memd}->set($mem_key, $data, $self->{expire});
	unless($mem) {
		my $msg = "failed to update a reminder session data in memcache. : sid=${sid}";
		FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
		croak $msg;
	}
	#
