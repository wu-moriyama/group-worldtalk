package FCC::Class::SellerSession;
################################################################################
...
sub new {
	...
	$self->{prefix} = "selmnu_";
	#cookie名
	$self->{cookie_name} = "ssid";
	$self->{auto_login_cookie_name} = "sauto";
	#
	bless $self, $class;
	return $self;
}
...
sub logoff {
	my($self) = @_;
	if($self->{data}) {
		my $seller_id = $self->{data}->{seller}->{seller_id};
		my $mem_key = $self->{prefix} . $seller_id;
		my $mem = $self->{memd}->delete($mem_key);
		unless($mem) {
			my $msg = "failed to delete a seller session data from memcache. : seller_id=${seller_id}";
...
#---------------------------------------------------------------------
sub create {
	my($self, $seller, $auto_login_enable) = @_;
	if( ! $seller || ref($seller) ne "HASH" ) {
		croak "the 1st argument must be a hashref.";
	}
	if( ! $seller->{seller_id} || $seller->{seller_id} =~ /[^\d]/ ) {
	...
	#memcacheにセット
	my $mem_key = $self->{prefix} . $seller->{seller_id};
	my $expire;
	...
	unless($mem) {
		my $msg = "failed to set a seller session data to memcache. : seller_id=$seller->{seller_id}";
		FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
		croak $msg;
	}
	#
...
sub recreate {
	...
	if($self->{data}) {
		...
		unless($mem2) {
			my $msg = "failed to set a seller session data to memcache. : seller_id=${seller_id}";
			FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
			croak $msg;
		}
		#
...
sub login_cookie_string {
	...
	my $seller_id = $data->{seller}->{seller_id};
...
sub update {
	...
	my $mem = $self->{memd}->set($mem_key, $data, $expire);
	unless($mem) {
		my $msg = "failed to update a seller session data in memcache. : seller_id=${seller_id}";
		FCC::Class::Log->new(conf=>$self->{conf})->loging("error", $msg);
		croak $msg;
	}
	#
