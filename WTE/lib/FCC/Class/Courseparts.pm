...
sub get_course_score_from_db {
	...
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("course_score", $list);
	return $list;
}

sub get_course_score2 {
	my($self) = @_;
	my $ref = $self->get_from_memcache("course_score2");
	unless($ref && ref($ref) eq "ARRAY") {
		$ref = $self->get_course_score2_from_db();
	}
	return $ref;
}

sub get_course_score2_from_db {
	my($self) = @_;
	#講師情報を検索
	my $ocourse = new FCC::Class::Course(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $params = {
		...
		limit => 20,
		offset => 0,
		sort => [ ['course_apply_deadline', 'ASC'], ['course_id', 'DESC'] ]
	};
	my $res = $ocourse->get_list($params);
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("course_score2", $list);
	return $list;
}

sub get_course_score3 {
	my($self) = @_;
	my $ref = $self->get_from_memcache("course_score3");
	unless($ref && ref($ref) eq "ARRAY") {
		$ref = $self->get_course_score3_from_db();
	}
	return $ref;
}

sub get_course_score3_from_db {
	my($self) = @_;
	#講師情報を検索
	my $ocourse = new FCC::Class::Course(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $params = {
	...
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("course_score3", $list);
	return $list;
}
