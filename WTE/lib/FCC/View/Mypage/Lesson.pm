    ...
    for my $k ( @{$names} ) {
        ...
        if ( $k eq "lsn_member_repo" ) {
            ...
            if ( $v eq "" ) {
                ...
                push( @errs, [ $k, "\"感想\" は140文字以内で入力してください。" ] );
            }
        }
    }
    return @errs;
...
sub get_receivable {
    ...
    #	my($amount) = $dbh->selectrow_array("SELECT SUM(lsn_prof_fee) FROM lessons WHERE member_id=${member_id} AND lsn_status=0 AND lsn_pay_type=${lsn_pay_type}");
    #	my($amount) = $dbh->selectrow_array("SELECT SUM(lsn_prof_fee) FROM lessons WHERE member_id=${member_id} AND lsn_charged_date=0 AND lsn_pay_type=${lsn_pay_type}");
    #---------------
...
sub get {
    ...
    #SELECT
    my $sql = "SELECT lessons.*, profs.* FROM lessons LEFT JOIN profs ON lessons.prof_id=profs.prof_id WHERE lessons.lsn_id=${lsn_id}";
    my $ref = $dbh->selectrow_hashref($sql);
    if ($ref) {
        $self->add_datetime_info($ref);
        $self->add_prof_info($ref);
...
sub get_csv {
    ...
    my $params = {};
    my @param_key_list =
      ( 'lsn_id', 'prof_id', 'member_id', 'seller_id', 'pdm_id', 'lsn_status', 'lsn_cancel', 'lsn_status_date_s', 'lsn_status_date_e', 'lsn_stime_s', 'lsn_stime_e', 'lsn_etime_s', 'lsn_etime_e', 'sort', 'charcode', 'returncode' );
    ...
    while ( my ( $k, $v ) = each %{$params} ) {
        if ( $k =~ /^(lsn|sch|prof|member|seller|pdm)_id$/ ) {
            if ( $v =~ /[^\d]/ ) {
                delete $params->{$k};
            }
            else {
...
sub get_list {
    ...
    #指定の検索条件を新たなhashrefに格納
    my $params         = {};
    my @param_key_list = ( 'lsn_id', 'prof_id', 'member_id', 'seller_id', 'pdm_id', 'lsn_status', 'lsn_cancel', 'lsn_status_date_s', 'lsn_status_date_e', 'lsn_stime_s', 'lsn_stime_e', 'lsn_etime_s', 'lsn_etime_e', 'offset', 'limit', 'sort', );
    if ( defined $in_params ) {
        for my $k (@param_key_list) {
            if ( defined $in_params->{$k} && $in_params->{$k} ne "" ) {
                $params->{$k} = $in_params->{$k};
    ...
    my @list;
    {
        my $sql = "SELECT lessons.*, profs.* FROM lessons LEFT JOIN profs ON lessons.prof_id=profs.prof_id";
        if (@wheres) {
            my $where = join( " AND ", @wheres );
            $sql .= " WHERE ${where}";
        }
