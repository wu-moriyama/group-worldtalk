package FCC::View::Site::IndexView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	for my $sch (@{$context->{sch_list}}) {
		...
		$hash->{epoch} = $epoch;
		$hash->{session_member_handle} = $session_member_handle;
