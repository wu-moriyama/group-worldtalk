package FCC::View::Admin::CpnactlstView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	for my $ref (@{$res->{list}}) {
		...
		while( my($k, $v) = each %{$ref} ) {
			...
			} elsif($k =~ /^(cpnact_type|cpnact_reason)$/) {
				$hash{"${k}_${v}"} = 1;
			}
		}
		if($ref->{cpnact_type} == 1) {
			$hash{cpnact_price_1} = abs($ref->{cpnact_price});
			$hash{cpnact_price_1_with_comma} = FCC::Class::String::Conv->new($hash{cpnact_price_1})->comma_format();
		} elsif($ref->{cpnact_type} == 2) {
			$hash{cpnact_price_2} = abs($ref->{cpnact_price});
			$hash{cpnact_price_2_with_comma} = FCC::Class::String::Conv->new($hash{cpnact_price_2})->comma_format();
		}
