package FCC::View::Admin::DsllstfrmView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	#ページナビゲーション
	my @navi_params = ('hit', 'fetch', 'start', 'end', 'next_num', 'prev_num', 'total_sale');
	...
	#検索対象の商品情報
	if( defined $res->{dwn} && ref($res->{dwn}) eq "HASH" ) {
		while( my($k, $v) = each %{$res->{dwn}} ) {
