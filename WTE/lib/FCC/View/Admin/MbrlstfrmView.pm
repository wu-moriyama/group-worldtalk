package FCC::View::Admin::MbrlstfrmView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	while( my($k, $v) = each %{$res->{params}} ) {
		if($k =~ /^(seller_id|member_id||member_handle|member_email)$/) {
			$t->param("s_${k}" => CGI::Utils->new()->escapeHtml($v));
		} elsif($k =~ /^member_(type|com|watcher|status)$/) {
			$t->param("s_${k}_${v}_selected" => 'selected="selected"');
			$t->param("s_${k}_${v}" => 1);
			if($v ne "") {
				$t->param("s_${k}_selected" => 1);
			}
		} elsif($k =~ /^member_award\d$/) {
			if($v == 1) {
				$t->param("s_${k}_checked" => 'checked="checked"');
				$t->param("s_${k}" => $v);
			}
		} elsif($k =~ /^(limit)$/) {
			$t->param($k => $v);
