package FCC::View::Admin::MbrdelfrmView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	#会員情報
	while( my($k, $v) = each %{$context->{proc}->{member}} ) {
		if( ! defined $v ) { $v = ""; }
		...
		} elsif($k =~ /^member_(gender|status|card|lang)$/) {
			$t->param("${k}_${v}" => 1);
		} elsif($k =~ /^member_(associate1|associate2|intro|comment|memo|memo2|note)$/) {
